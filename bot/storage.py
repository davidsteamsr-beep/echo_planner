...
async def find_user_id_by_token(token: str) -> int | None:
    """Найти user_id по shortcut_token. None если не найден."""
    ...
    try:
        ...
        row = await cur.fetchone()
        if row:
            return int(row[0] if not hasattr(row, "keys") else row["user_id"])
    except Exception:
        # fallback: scan
        cur = await db.execute("SELECT user_id, data FROM users")
        rows = await cur.fetchall()
        for r in rows:
            uid = r[0] if not hasattr(r, "keys") else r["user_id"]
            raw = r[1] if not hasattr(r, "keys") else r["data"]
            try:
                import json as _json
                d = _json.loads(raw) if isinstance(raw, str) else raw
                if d.get("shortcut_token") == token:
                    return int(uid)
            except Exception:
                continue
    return None
