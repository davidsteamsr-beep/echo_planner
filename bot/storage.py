...
async def ensure_partner_benefits(user_id: int, username: str | None = None) -> dict:
    """Партнёр (traffer): role + подписка Навсегда."""
    data = await load_user(user_id)
    uname = (username or data.get("username") or "").lstrip("@").lower()
    tr = None
    if uname:
        tr = await get_traffer_by_username(uname)
    if not tr and data.get("role") == "traffer":
        # already traffer
        pass
    elif tr:
        data["role"] = "traffer"
        data["traffer_code"] = tr.get("code")
        data["username"] = uname or data.get("username")
    else:
        return data

    if data.get("role") == "traffer":
        sub = data.setdefault("subscription", {})
        sub["active"] = True
        sub["type"] = "lifetime"
        sub["until"] = None
