...
def create_api_app() -> FastAPI:
    ...
    @app.get("/api/me")
    async def me(x_telegram_init_data: str = Header(None)):
        user = validate_init_data(x_telegram_init_data or "")
        if not user:
            # Для локальной разработки пропускаем
    ...
    @app.post("/api/arthur")
    async def arthur(request: Request, x_telegram_init_data: str = Header(None)):
    ...
    async def toggle_task(request: Request, x_telegram_init_data: str = Header(None)):
        body = await request.json()
        user = validate_init_data(x_telegram_init_data or "")
        user_id = user["id"] if user else 0
