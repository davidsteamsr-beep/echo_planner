...
START_TEXT = (
    "ECHO Planner — это голосовой трекер, который создан для того, чтобы убрать лишние действия из твоей жизни.\n\n"
    "Вместо того чтобы открывать приложение, выбирать категорию и заполнять поля, ты просто говоришь. Бот сам понимает, что это — расход, встреча, задача или приём пищи, и сохраняет в нужный раздел.\n\n"
    "Это работает через Telegram. Не нужно ничего устанавливать, настраивать или запоминать. Ты можешь записывать идеи, траты или планы одной фразой. Даже если ты не открывал телефон — с помощью Action Button на iPhone или касания по задней крышке ты можешь начать запись в один клик.\n\n"
...
def start_kb() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="SUPPORT", url="https://t.me/dvvsww")],
    ])
...
async def safe_answer(call: CallbackQuery, text: str | None = None, show_alert: bool = False):
    """answerCallbackQuery без падения на просроченном query (cold start / retry)."""
    try:
        if text is not None:
...
def guides_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🎬 ACTION BUTTON", callback_data="guide_action")],
        [InlineKeyboardButton(text="🎬 КАСАНИЕ ПО ЗАДНЕЙ КРЫШКЕ", callback_data="guide_backtap")],
        [InlineKeyboardButton(text="← ВЕРНУТЬСЯ", callback_data="back_start")],
    ])


def guide_video_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="← ВЕРНУТЬСЯ", callback_data="guides")],
    ])


GUIDES_TEXT = """Action Button — эксклюзивная фишка iPhone 15 Pro и новее. Настрой её на быстрый запуск ECHO Planner: просто нажми и говори. Никаких лишних движений — от идеи до записи за секунду.

Для всех остальных iPhone. Настрой двойное или тройное касание по задней крышке. Это работает на любом iPhone с iOS 14 и новее. Настрой один раз — пользуйся всегда.
...
async def send_guides_screen(target_message: Message):
    await target_message.answer(GUIDES_TEXT, reply_markup=guides_kb(), parse_mode=None)


async def send_guide_video(target_message: Message, kind: str):
    names = {
        "action": ("action_button.mp4", "Action Button"),
        "backtap": ("back_tap.mp4", "Касание по задней крышке"),
    }
    filename, title = names[kind]
    path = ASSETS_DIR / "guides" / filename
    caption = f"🎬 {title}"
    if path.exists():
        await target_message.answer_video(
            FSInputFile(path),
            caption=caption,
            reply_markup=guide_video_kb(),
        )
    else:
        await target_message.answer(
            caption + "\n\nВидео пока не загружено (bot/assets/guides/" + filename + ").",
            reply_markup=guide_video_kb(),
        )
...
async def cmd_start(message: Message):
    ...
    await storage.update_profile(uid, uname, fname)
    await storage.ensure_owner(uid, uname)
    await storage.ensure_partner_benefits(uid, uname)
    tr0 = await storage.get_traffer_by_username(uname or "")
    if tr0:
        _d = await storage.load_user(uid)
        _d["role"] = "traffer"
        _d["traffer_code"] = tr0["code"]
        await storage.save_user(uid, _d)
    ...
    if pl.startswith("ref_"):
        code = payload[4:]  # preserve case of code
        await storage.set_referred_by(uid, code)
        # promote traffer role if username matches
        tr = await storage.get_traffer_by_username(uname or "")
        if tr:
            data = await storage.load_user(uid)
            data["role"] = "traffer"
            data["traffer_code"] = tr["code"]
            await storage.save_user(uid, data)

    if pl in ("subscribe", "tariffs", "pay", "pro"):
        await send_tariffs_screen(message)
        return
    await send_start_screen(message, user_id=uid)
...
async def cb_guides(call: CallbackQuery):
    await safe_answer(call)
    await safe_delete(call.message)
    await send_guides_screen(call.message)


@router.callback_query(F.data == "guide_action")
async def cb_guide_action(call: CallbackQuery):
    await safe_answer(call)
    await safe_delete(call.message)
    await send_guide_video(call.message, "action")


@router.callback_query(F.data == "guide_backtap")
async def cb_guide_backtap(call: CallbackQuery):
    await safe_answer(call)
    await safe_delete(call.message)
    await send_guide_video(call.message, "backtap")


@router.callback_query(F.data == "open_app")
async def cb_open_app(call: CallbackQuery):
    await safe_answer(call, "Задай WEBAPP_URL в .env", show_alert=True)
...
async def successful_payment(message: Message):
    ...
    if payload.startswith("monthly_"):
        ...
        await storage.record_payment(user_id, "monthly", sp.telegram_payment_charge_id)
        ...
        await storage.record_payment(user_id, "lifetime", sp.telegram_payment_charge_id)
