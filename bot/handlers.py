...
START_TEXT = (
    ...
    "Вместо заполнения таблиц, форм или выбора категорий, а"
    "бот определяет тип записи, извлекает "
...
async def send_start_screen(target_message: Message):
    """Стартовый экран: фото + текст + кнопки ECHO PLANNER / ТАРИФЫ."""
    ...
    if photo.exists():
        ...
        await target_message.answer(START_TEXT, reply_markup=start_kb())


async def send_tariffs_screen(target_message: Message):
    """Экран тарифов: фото + текст + кнопки планов + ВЕРНУТЬСЯ."""
...
async def cb_back_start(call: CallbackQuery):
    """С экрана тарифов — обратно на стартовое сообщение."""
    await call.answer()
    try:
        await call.message.delete()
    except Exception:
        pass
    await send_start_screen(call.message)


@router.callback_query(F.data == "open_app")
async def cb_open_app(call: CallbackQuery):
    await call.answer(
        "Задай WEBAPP_URL в .env (ссылка на мини-приложение)",
        show_alert=True,
    )
...
async def cb_plan_trial(call: CallbackQuery):
    ...
    if st.get("reason") == "trial_ended" or data.get("subscription", {}).get("trial_started"):
        # trial уже был
        if not st.get("ok"):
...
async def _send_invoice(call: CallbackQuery, bot: Bot, kind: str):
    if not PAYMENT_PROVIDER_TOKEN:
        await call.message.answer(
            ...
            "BotFather → Payments → ЮKassa → provider token → PAYMENT_PROVIDER_TOKEN в .env\n\n"
            "Для теста: /activate_test",
        )
        return

    if kind == "monthly":
        prices = [LabeledPrice(label="Подписка Pro · 30 дней", amount=PRICE_MONTHLY)]
        await bot.send_invoice(
            chat_id=call.from_user.id,
            title="ECHO Planner Pro",
            description="Основная подписка на 30 дней. Неограниченные запросы, статистика, ARTHUR.",
        ...
        await bot.send_invoice(
            ...
            description="Все функции Pro без срока. Один платёж.",
...
async def successful_payment(message: Message):
    ...
    if payload.startswith("monthly_"):
        ...
        await storage.set_subscription(user_id, "monthly", until=until, payment_id=sp.telegram_payment_charge_id)
        await message.answer("Оплата прошла. Подписка Pro активна 30 дней.", reply_markup=app_kb())
    elif payload.startswith("lifetime_"):
        await storage.set_subscription(user_id, "lifetime", until=None, payment_id=sp.telegram_payment_charge_id)
...
async def activate_test(message: Message):
    ...
    await message.answer("✅ Тестовая подписка Pro на 30 дней активирована.", reply_markup=app_kb())
...
async def handle_note(message: Message, bot: Bot):
    ...
    if message.voice:
        await message.answer(
            "🎤 Голос в MVP — через мини-приложение (Артур → микрофон). Или напиши текстом.",
            reply_markup=app_kb(),
        )
        return
